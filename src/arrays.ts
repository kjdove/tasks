...
export function bookEndList(numbers: number[]): number[] {
    ...
    if (numbers.length === 0) {
        ...
        retArr = [numbers[0], numbers[0]];
        return retArr;
    } else {
        let len: number = numbers.length;
        retArr = [numbers[0], numbers[len - 1]];
        return retArr;
    }
}
...
export const removeDollars = (amounts: string[]): number[] => {
    ...
    let retArr: number[] = remDS.map((str: string): number =>
        isNaN(Number(str)) ? 0 : Number(str),
    );
    return retArr;
};
...
export function allRGB(colors: string[]): boolean {
    ...
    let retColors: string[] = colors.filter(
        (c: string): boolean => c === "red" || c === "blue" || c === "green",
    );
    if (retColors.length === colors.length) {
        return true;
    }
    return false;
}
